...
Unlike the Simple Moving Average (SMA), which assigns equal weight to all data points in the period, the EMA applies an exponentially decreasing multiplier to older prices. This weighting mechanism reduces the indicator's lag, allowing it to react faster to sudden price shifts, breakaways, and trend reversals.

## 3. Formulas

The EMA is calculated in three steps:

1. **Calculate the Simple Moving Average (Initial Seed):** $$SMA_0 = \frac{\sum_{i=1}^{N} Close_i}{N}$$
    
2. **Calculate the Multiplier (Smoothing Factor K):** $$K = \frac{2}{N+1}$$ _(where N is the designated EMA period length, e.g., 9, 20, 50, 200)_
    
3. **Calculate the Current Period EMA:** $$EMA_t = (Price_t \times K) + (EMA_{t-1} \times (1-K))$$
...
|Common Periods|**9/20:** Short-term momentum & scalp triggers · **50:** Medium-term trend benchmark · **200:** Macro trend regime line|
...
- Multi-EMA Ribbon Trend Alignment (9/20/50/200 "fan out" confirmation)
- EMA Pullback Entry System (buying dips into a rising EMA within an established uptrend)
- Golden Cross / Death Cross Positional System
